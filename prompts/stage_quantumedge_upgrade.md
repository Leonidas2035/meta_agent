# STAGE: — ML + Signals + Risk + Execution (4-layer upgrade)

## Objective

Upgrade the QuantumEdge (ai_scalper_bot) trading system to a **v2 architecture** with:
1 multi-horizon & multi-timeframe ML signals,
2 robust signal / regime logic,
3 professional money & risk management,
4 execution / scalp-mode logic tailored for low-latency futures scalping.

The goal is NOT just to patch small bugs, but to propose and implement **high-quality code structures** that reflect how serious HFT / scalping bots are built, while keeping the current project runnable and reasonably simple to maintain.

## Current State (high level)

- ML: single-horizon XGBoost SignalModel + simple Ensemble over limited horizons; online features are basic returns/volumes; no market regime features.
- Signals: DecisionEngine mostly uses p_up>0.5 thresholds and fixed size; no strong filters, no regime awareness.
- Risk: position sizing is simplistic (risk_per_trade * leverage), no edge-based sizing, no daily/session limits, no max trades per hour.
- Execution: demo executor for Binance futures testnet; fixed SL/TP style; no scalp-mode, weak integration with orderbook depth/spread; order-manager / risk-engine are mostly stubs.

(See: bot/ml/*, bot/engine/decision_engine.py, bot/trading/*, bot/market_data/*, bot/ai/risk_moderator.py, config/settings.yaml, config/pairs.yaml.)

## Target Architecture (4 layers)

### LAYER 1 — ML / Features

- Multi-horizon models for at least horizons: **1, 5, 30** (seconds or tick bars).
- Multi-timeframe features: 1s / 5s / 30s / 1m with volatility, VWAP, order-flow, imbalance, volume spikes, simple regime tags.
- Warmup / context: minimal required history (e.g. 5–10 minutes of data) before any trading decisions are allowed.

### LAYER 2 — Decision & Signal Logic

- Signal filter on top of raw ML:
  - agreement between horizons (e.g. h1 & h5 must align, h30 must not be strongly opposite),
  - confidence thresholds (p_up > 0.55–0.6, configurable),
  - recent loss-streak / over-trading filter (pause after N losing trades in a short window).
- Regime-aware logic:
  - flat regime → scalp (small TP/SL, small size, more frequent),
  - trend regime → longer-living positions, fewer flips.

### LAYER 3 — Money & Risk Management

- Position sizing based on **edge**:
  - base_size * f(edge), where edge ≈ |p_up − 0.5|.
  - enforce min_size > 0 and max_size capped by configuration & Supervisor-style limits.
- Volatility-aware SL/TP:
  - high vol → wider stops, smaller sizes;
  - low vol → tighter stops, larger sizes within limits.
- Session constraints:
  - max trades per hour,
  - max daily loss (absolute and %),
  - hooks to signal an upper-layer Supervisor (even if currently stubbed).

### LAYER 4 — Execution / Scalp-mode

- A dedicated **scalp-mode** in DecisionEngine / execution layer, switchable via config:
  - stricter entry conditions (p_up > threshold + order-flow confirmation),
  - small TP/SL tuned for micro-moves,
  - checks on spread and orderbook depth:
    - no trades if spread > X bps,
    - no trades if local depth < configured threshold.
- Order policy:
  - use market only when spread & depth are acceptable and latency is low;
  - otherwise prefer passive/near-touch limit orders with small offsets and timeouts;
  - safe handling of partial fills and cancellations.
